...
$(function() {
    ...
    function setBrandList(dom, brandtitleid, callback) {
        $.ajax({
            url: "http://127.0.0.1:9090/api/getbrand" + "?timestamp=" + new Date().getTime(),
    ...
    function setBrandProduct(dom, brandtitleid, callback) {
        $.ajax({
            url: "http://127.0.0.1:9090/api/getbrandproductlist" + "?timestamp=" + new Date().getTime(),
    ...
    function setProductCom(dom, productlist, callback) {
        ...
        $.ajax({
            url: "http://127.0.0.1:9090/api/getproduct" + "?timestamp=" + new Date().getTime(),
            data: { "productid": productid[0] },
            success: function(result) {
                result = result.result;
                for (var j = 0; j < result.length; j++) {
                    $.ajax({
                        url: "http://127.0.0.1:9090/api/getproductcom" + "?timestamp=" + new Date().getTime(),
                        data: { "productid": productid[0] },
                        success: function(data) {
                            data = data.result;
                            var productCom = []
